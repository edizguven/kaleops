<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Operatör Paneli') }} - <span class="text-blue-600 uppercase">{{ auth()->user()->role }} İstasyonu</span>
        </h2>
    </x-slot>
                ...
                @forelse($jobs as $job)
                    <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg border-l-8 border-blue-500 transition hover:shadow-2xl">
                        <div class="p-6">
                            <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
                                ...
                                <div class="flex flex-wrap gap-2 mt-4 md:mt-0">
                                    @forelse($job->jobFiles as $f)
                                        <a href="{{ route('jobfile.download', $f) }}" class="flex items-center px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-blue-600 hover:text-white transition font-bold text-sm border border-gray-300" download>
                            ...
                            @if(auth()->user()->role == 'cam')
                                <form action="{{ route('operator.update', $job->id) }}" method="POST" class="bg-blue-50 p-6 rounded-xl border border-blue-100">
                                    @csrf
                                    <div class="space-y-4">
                            ...
                            @if(auth()->user()->role == 'lazer')
                                <form action="{{ route('operator.update', $job->id) }}" method="POST" class="bg-red-50 p-6 rounded-xl border border-red-100">
                                    @csrf
                                    <div class="space-y-4">
                                        <div class="flex flex-col md:flex-row items-end gap-4">
                                            <div class="w-full">
                            ...
                            @if(auth()->user()->role == 'cmm')
                                <form action="{{ route('operator.update', $job->id) }}" method="POST" class="bg-purple-50 p-6 rounded-xl border border-purple-100">
                                    @csrf
                                    <div class="space-y-4">
                                        <div class="flex flex-col md:flex-row items-end gap-4">
                                            <div class="w-full">
                            ...
                            @if(auth()->user()->role == 'tesviye')
                                <form action="{{ route('operator.update', $job->id) }}" method="POST" class="bg-orange-50 p-6 rounded-xl border border-orange-100">
                                    @csrf
                                    <div class="space-y-4">
                                        <div class="flex flex-col md:flex-row items-end gap-4">
                                            <div class="w-full">
                            ...
                            @if(auth()->user()->role == 'torna')
                                <form action="{{ route('operator.update', $job->id) }}" method="POST" class="bg-cyan-50 p-6 rounded-xl border border-cyan-100">
                                    @csrf
                                    <div class="space-y-4">
                                        <div class="flex flex-col md:flex-row items-end gap-4">
                                            <div class="w-full">
                            ...
                            @if(auth()->user()->role == 'planning')
                                <div class="mb-6 p-4 bg-gray-50 rounded-xl border border-gray-200">
                                    ...
                                    <div class="grid grid-cols-2 md:grid-cols-5 gap-3 text-sm">
                                        <div><span class="text-gray-500">CAM:</span> <span class="font-bold">{{ $job->cam_minutes ?? '--' }} dk</span></div>
                                        <div><span class="text-gray-500">Lazer:</span> <span class="font-bold">{{ $job->lazer_minutes ?? '--' }} dk</span></div>
                                        <div><span class="text-gray-500">CMM:</span> <span class="font-bold">{{ $job->cmm_minutes ?? '--' }} dk</span></div>
                                        <div><span class="text-gray-500">Tesviye:</span> <span class="font-bold">{{ $job->tesviye_minutes ?? '--' }} dk</span></div>
                                        <div><span class="text-gray-500">Torna:</span> <span class="font-bold">{{ $job->torna_minutes ?? '--' }} dk</span></div>
                                    </div>
                                    <p class="text-xs text-gray-400 mt-2">Toplam: {{ $job->total_minutes }} dk</p>
                                </div>
                                <form action="{{ route('operator.update', $job->id) }}" method="POST" class="bg-indigo-50 p-6 rounded-xl border border-indigo-100">
                                    @csrf
                                    <div class="space-y-4">
                                        <div class="flex flex-col md:flex-row items-end gap-4">
                                            <div class="w-full">
                            ...
                            @if(auth()->user()->role == 'packaging')
                                <form action="{{ route('operator.update', $job->id) }}" method="POST" class="bg-pink-50 p-6 rounded-xl border border-pink-100">
                                    @csrf
                                    <div class="flex flex-col md:flex-row items-end gap-4">
                                        <div class="w-full">
                            ...
                            @if(auth()->user()->role == 'logistics')
                                <form action="{{ route('operator.update', $job->id) }}" method="POST" enctype="multipart/form-data" class="bg-teal-50 p-6 rounded-xl border border-teal-100">
                                    @csrf
                            ...
                            @if(auth()->user()->role == 'accounting')
                                <form action="{{ route('operator.update', $job->id) }}" method="POST" enctype="multipart/form-data" class="bg-green-50 p-6 rounded-xl border border-green-100">
                                    @csrf
