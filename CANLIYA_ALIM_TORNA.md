...
**Hata alırsanız:** `assign_tesviye` yoksa önce mevcut “assign_*” migration’ınızı (CANLIYA_ALIM.md) çalıştırmış olun. Sütun adı farklıysa `AFTER assign_tesviye` kısmını kendi son sütununuza göre değiştirin (örn. `AFTER current_stage`).
